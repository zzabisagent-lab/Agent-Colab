...
- Target commit: `a488dfe25ab39358e207307ecc394729ae8f403f` (this checkout is a read-only worktree at exactly that commit; do not modify product code — validation plan §2.5)
...
- Working root: this worktree (git commit a488dfe25ab39358e207307ecc394729ae8f403f). `uv` environment synced; run commands with `uv run ...` or `make ...` (PATH includes ~/.local/bin).
...
6. Write your report to `verification/phase-4/VR-P4-001.yaml` as YAML in exactly the structure of validation plan §6.2
   (fields: verification_id `VR-P4-001`, criteria_version `v8.0`, implementer_agent_id
   ...
   target_commit `a488dfe25ab39358e207307ecc394729ae8f403f`, environment_fingerprint, started_at, completed_at, tests (one entry per
   mandatory Test ID with result PASS/FAIL/NOT_RUN/NOT_APPLICABLE, evidence_ref pointing to files
   you wrote under `verification/phase-4/evidence-r001/` or to repository paths, and a note), findings (list),
...
7. Save raw evidence you produce (command outputs with exit codes, query results, hashes) under
   `verification/phase-4/evidence-r001/`. Never include secret values in the report or evidence.

Rules: you may run any read-only command, tests, linters, and database queries; you must not edit
files outside `verification/phase-4/evidence-r001/` and `verification/phase-4/VR-P4-001.yaml`; you must not fix product code; you must not
