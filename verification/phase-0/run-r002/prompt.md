...
- Target commit: `9f7f1270db54b9bae3ed375a1861fc16652fb92b` (this checkout is a read-only worktree at exactly that commit; do not modify product code — validation plan §2.5)
...
- Working root: this worktree (git commit 9f7f1270db54b9bae3ed375a1861fc16652fb92b). `uv` environment synced; run commands with `uv run ...` or `make ...` (PATH includes ~/.local/bin).
...
- No Docker/container runtime on this host; no root. Record Tests needing it as NOT_RUN.
...
8. `Spike evidence: docs/protocol/mattermost-spike.md, docs/protocol/mcp-spike.md, evidence/phase-0/spikes/; re-run Mattermost via scripts/dev/mattermost-local.sh and MCP via spikes/mcp/ (see docs)`
9. `docker compose up -d --wait  (V-P0-04; requires Docker, unavailable on the build host)`
...
6. Write your report to `verification/phase-0/VR-P0-002.yaml` as YAML in exactly the structure of validation plan §6.2
   (fields: verification_id `VR-P0-002`, criteria_version `v8.0`, implementer_agent_id
   ...
   target_commit `9f7f1270db54b9bae3ed375a1861fc16652fb92b`, environment_fingerprint, started_at, completed_at, tests (one entry per
   mandatory Test ID with result PASS/FAIL/NOT_RUN/NOT_APPLICABLE, evidence_ref pointing to files
   you wrote under `verification/phase-0/evidence-r002/` or to repository paths, and a note), findings (list),
...
7. Save raw evidence you produce (command outputs with exit codes, query results, hashes) under
   `verification/phase-0/evidence-r002/`. Never include secret values in the report or evidence.

Rules: you may run any read-only command, tests, linters, and database queries; you must not edit
files outside `verification/phase-0/evidence-r002/` and `verification/phase-0/VR-P0-002.yaml`; you must not fix product code; you must not
