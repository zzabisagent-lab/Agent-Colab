...
- Target commit: `4f2369f49abf78095a6281a6b0d498b47074ad17` (this checkout is a read-only worktree at exactly that commit; do not modify product code — validation plan §2.5)
...
- Working root: this worktree (git commit 4f2369f49abf78095a6281a6b0d498b47074ad17). `uv` environment synced; run commands with `uv run ...` or `make ...` (PATH includes ~/.local/bin).
...
- Test-environment credentials are exported to your process as environment variables (never print or record their values): ADMIN_PASSWORD, ADMIN_TOKEN, ADMIN_USER, BOT_TOKEN, BOT_USER_ID, MATTERMOST_ADMIN2_PASSWORD, MATTERMOST_ADMIN2_USERNAME, TELEGRAM_BOT_TOKEN, TELEGRAM_TEST_CHAT_A, TELEGRAM_TEST_CHAT_B.
...
4. `Every V-P7 Test has a SELF evidence attempt under evidence/phase-7/SELF-V-P7-NN/attempt-NN/result.json; the latest attempt of each is PASS`
...
11. `Report and deployment decision (V-P7-18): uv run python -m tools.deployment_decision --check; uv run pytest tests/e2e/test_release_evidence.py -q`
...
6. Write your report to `verification/phase-7/VR-P7-002.yaml` as YAML in exactly the structure of validation plan §6.2
   (fields: verification_id `VR-P7-002`, criteria_version `v8.0`, implementer_agent_id
   ...
   target_commit `4f2369f49abf78095a6281a6b0d498b47074ad17`, environment_fingerprint, started_at, completed_at, tests (one entry per
   mandatory Test ID with result PASS/FAIL/NOT_RUN/NOT_APPLICABLE, evidence_ref pointing to files
   you wrote under `verification/phase-7/evidence-r002/` or to repository paths, and a note), findings (list),
...
7. Save raw evidence you produce (command outputs with exit codes, query results, hashes) under
   `verification/phase-7/evidence-r002/`. Never include secret values in the report or evidence.

Rules: you may run any read-only command, tests, linters, and database queries; you must not edit
files outside `verification/phase-7/evidence-r002/` and `verification/phase-7/VR-P7-002.yaml`; you must not fix product code; you must not
