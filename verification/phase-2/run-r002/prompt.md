# Independent Phase Verification — Agent-Colab Phase 2
...
- Validation plan (your authority): `docs/baseline/agent-colab-validation-plan_en-v8.md` — Phase 2 section, plus §2, §4, §6, §7
- Applicable ADRs: `docs/adr/`
- Target commit: `88adc56791b08285330a50bb9c4b8dad897d6b3f` (this checkout is a read-only worktree at exactly that commit; do not modify product code — validation plan §2.5)
- Implementer Evidence Manifest: `evidence/phase-2/manifest.yaml` (self-report; it may be consulted as evidence but never decides PASS)
- Self-test evidence: `evidence/phase-2/SELF-*/`
- Clean-environment reproduction steps: see below
- Known gaps declared by the implementer: see the manifest `known_gaps`

## Environment

- Working root: this worktree (git commit 88adc56791b08285330a50bb9c4b8dad897d6b3f). `uv` environment synced; run commands with `uv run ...` or `make ...` (PATH includes ~/.local/bin).
...
2. `export PATH=$HOME/.local/bin:$PATH; export AGENT_COLAB_TEST_DATABASE_URL=postgresql://colab@127.0.0.1:54329/colab_test (any empty PostgreSQL 16 maintenance DB; the test session creates and drops its own database and runs migrations 0001-0007)`
3. `make bootstrap && make lint && make test && make check-docs && make build   # make build includes the web console (pnpm)`
4. `Every V-P2 Test has a SELF evidence attempt under evidence/phase-2/SELF-V-P2-NN/attempt-NN/result.json listing the exact command; the latest attempt of each ID is PASS`
5. `Bridge rules and relay: uv run pytest tests/integration/test_bridge_db.py tests/unit/test_bridge_rules.py -q -s   (V-P2-03..08/10/13/14/15/17); live Telegram: uv run pytest tests/integration/test_bridge_live.py -q (needs TELEGRAM_BOT_TOKEN, TELEGRAM_TEST_CHAT_A/B)`
6. `Mattermost provider (local TE): scripts/dev/mattermost-local.sh start; uv run pytest tests/integration/test_mattermost_provider.py -q -rs   (V-P2-01, V-P2-19)`
7. `Bridge Admin UI (V-P2-12): cd web-admin && pnpm install && pnpm run build; PLAYWRIGHT_CHROMIUM_EXECUTABLE=<chromium> uv run pytest tests/e2e/test_admin_bridges_ui.py -q`
8. `Command grammar / cards / actions / i18n: uv run pytest tests/unit/test_command_router.py tests/integration/test_task_cards.py tests/integration/test_actions_db.py tests/integration/test_i18n_channels.py -q`
9. `External identity, link challenge, channel config/lifecycle: uv run pytest tests/integration/test_external_identity_commands.py tests/unit/test_link_challenge.py tests/integration/test_channel_config_db.py -q`
10. `Messages/retention and notifications: uv run pytest tests/integration/test_message_retention.py tests/integration/test_notification_delivery.py tests/unit/test_notification_providers.py -q`

## Your task

1. Read the three baseline documents' relevant sections and the Phase 2 Test table
   (`V-P2-01` … `V-P2-31`). Every Test ID in that table is mandatory.
...
6. Write your report to `verification/phase-2/VR-P2-002.yaml` as YAML in exactly the structure of validation plan §6.2
   (fields: verification_id `VR-P2-002`, criteria_version `v8.0`, implementer_agent_id
   ...
   `sha256:ecbd74ce21cb00f0e36136b78a0fe57c8241013e4125100e2871351c90b530b9`, identity_graph_version `identity-v8-001`, effective_policy_hash `sha256:1ea0e18ddab4cd44770830dc362978d621f6c2b5f6af30277506d6fcf1624c6b`,
   target_commit `88adc56791b08285330a50bb9c4b8dad897d6b3f`, environment_fingerprint, started_at, completed_at, tests (one entry per
   mandatory Test ID with result PASS/FAIL/NOT_RUN/NOT_APPLICABLE, evidence_ref pointing to files
   you wrote under `verification/phase-2/evidence-r002/` or to repository paths, and a note), findings (list),
...
7. Save raw evidence you produce (command outputs with exit codes, query results, hashes) under
   `verification/phase-2/evidence-r002/`. Never include secret values in the report or evidence.

Rules: you may run any read-only command, tests, linters, and database queries; you must not edit
files outside `verification/phase-2/evidence-r002/` and `verification/phase-2/VR-P2-002.yaml`; you must not fix product code; you must not
