...
- Target commit: `8680e799277790b11fa9fc858f53e678888cfb46` (this checkout is a read-only worktree at exactly that commit; do not modify product code — validation plan §2.5)
...
- Working root: this worktree (git commit 8680e799277790b11fa9fc858f53e678888cfb46). `uv` environment synced; run commands with `uv run ...` or `make ...` (PATH includes ~/.local/bin).
...
6. Write your report to `verification/phase-2/VR-P2-001.yaml` as YAML in exactly the structure of validation plan §6.2
   (fields: verification_id `VR-P2-001`, criteria_version `v8.0`, implementer_agent_id
   ...
   target_commit `8680e799277790b11fa9fc858f53e678888cfb46`, environment_fingerprint, started_at, completed_at, tests (one entry per
   mandatory Test ID with result PASS/FAIL/NOT_RUN/NOT_APPLICABLE, evidence_ref pointing to files
   you wrote under `verification/phase-2/evidence-r001/` or to repository paths, and a note), findings (list),
...
7. Save raw evidence you produce (command outputs with exit codes, query results, hashes) under
   `verification/phase-2/evidence-r001/`. Never include secret values in the report or evidence.

Rules: you may run any read-only command, tests, linters, and database queries; you must not edit
files outside `verification/phase-2/evidence-r001/` and `verification/phase-2/VR-P2-001.yaml`; you must not fix product code; you must not
