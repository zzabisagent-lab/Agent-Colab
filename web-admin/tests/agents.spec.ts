...
test('administrator commits a Role version and previews effective permissions', async ({ page }) => {
  ...
  await page.getByLabel('Role id').fill('role-ui-reviewer')
  ...
  await page.getByLabel('Explicit deny (comma separated)').fill('task.delete')
  ...
  await page.getByLabel('Role id', { exact: true }).last().fill('role-ui-reviewer')
  ...
  await expect(preview).toContainText('task.delete')
})
