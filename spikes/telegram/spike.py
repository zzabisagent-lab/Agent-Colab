        ...
        if isinstance(v, dict):
            ...
            for k, x in v.items():
                if k in {"title", "username", "first_name", "last_name", "invite_link"}:
                    ...
                    out[k] = {"is_bot": x.get("is_bot"), "id": "<bot-id>" if str(x.get("id")) == self.bot_id else "<user-id>"}
...
class Spike:
    ...
    async def run(self) -> None:
        ...
        steps["getMe"] = {"status": st, "ok": me.get("ok"), "is_bot": me.get("result", {}).get("is_bot")}
        obs["bot_can_join_groups"] = me.get("result", {}).get("can_join_groups")
        obs["bot_can_read_all_group_messages"] = me.get("result", {}).get("can_read_all_group_messages")
        for chat in (self.chat_a, self.chat_b):
            st, c, _ = await self.call("getChat", chat_id=chat)
            res = c.get("result", {})
            steps[f"getChat:{self.label(chat)}"] = {
                "status": st, "type": res.get("type"), "is_forum": res.get("is_forum"),
            }
            ...
            steps[f"getChatMember:{self.label(chat)}"] = {"status": st, "member_status": mres.get("status"), "rights": rights}
            obs[f"rights:{self.label(chat)}"] = {"status": mres.get("status"), **rights}
        # (b) topic in chat A, messages, reply, edit
        self.step = "b-topic-A"
        stamp = dt.datetime.now(dt.UTC).strftime("%Y%m%dT%H%M%SZ")
        st, t, _ = await self.call("createForumTopic", chat_id=self.chat_a, name=f"agent-colab-spike-{stamp}")
        topic_a = t.get("result", {}).get("message_thread_id")
        steps["createForumTopic:chat-A"] = {"status": st, "ok": t.get("ok"), "message_thread_id": topic_a, "error": t.get("description")}
        if topic_a:
            self.topics.append((self.chat_a, topic_a))
            st, m1, _ = await self.call("sendMessage", chat_id=self.chat_a, message_thread_id=topic_a, text="spike root message")
            r1 = m1.get("result", {})
            steps["sendMessage:topic-A"] = {"status": st, "message_id": r1.get("message_id"), "message_thread_id": r1.get("message_thread_id"), "is_topic_message": r1.get("is_topic_message")}
            obs["topic_message_fields"] = {k: r1.get(k) for k in ("message_thread_id", "is_topic_message")}
            ...
            if r1.get("message_id"):
                ...
                st, m2, _ = await self.call(
                    "sendMessage", chat_id=self.chat_a, message_thread_id=topic_a, text="spike reply",
                    reply_parameters={"message_id": r1["message_id"]},
                )
                r2 = m2.get("result", {})
                rt = r2.get("reply_to_message", {})
                steps["sendMessage:reply-in-topic-A"] = {
                    "status": st, "message_id": r2.get("message_id"), "message_thread_id": r2.get("message_thread_id"),
                    "reply_to_message_id": rt.get("message_id"), "reply_to_is_topic_message": rt.get("is_topic_message"),
                }
                ...
                st, e1, _ = await self.call("editMessageText", chat_id=self.chat_a, message_id=r1["message_id"], text="spike root message (edited)")
                er = e1.get("result", {})
                steps["editMessageText:own-A"] = {"status": st, "ok": e1.get("ok"), "edit_date_present": "edit_date" in er, "message_thread_id": er.get("message_thread_id")}
                obs["edit_own_message_possible"] = bool(e1.get("ok"))
                # unchanged text edit -> documented 400 "message is not modified"
                st, e2, _ = await self.call("editMessageText", chat_id=self.chat_a, message_id=r1["message_id"], text="spike root message (edited)")
                steps["editMessageText:unchanged-A"] = {"status": st, "ok": e2.get("ok"), "description": e2.get("description")}
            # topic service message: the topic's thread id is the id of the forum_topic_created service message
            st, s1, _ = await self.call("sendMessage", chat_id=self.chat_a, message_thread_id=topic_a, text="probe reply to topic service message", reply_parameters={"message_id": topic_a})
            sr = s1.get("result", {})
            rt = sr.get("reply_to_message", {})
            steps["sendMessage:reply-to-topic-service-message-A"] = {
                "status": st, "ok": s1.get("ok"), "reply_to_message_id": rt.get("message_id"),
                "reply_to_forum_topic_created": rt.get("forum_topic_created"), "description": s1.get("description"),
            }
            obs["topic_thread_id_equals_first_message_id"] = bool(rt) and rt.get("message_id") == topic_a
            obs["topic_creating_message_is_forum_topic_created"] = bool(rt.get("forum_topic_created"))
            if sr.get("message_id"):
                self.created.append((self.chat_a, sr["message_id"]))
            # edit a message not owned by the bot: the topic service message (owned by the group/creator)
            st, e3, _ = await self.call("editMessageText", chat_id=self.chat_a, message_id=topic_a, text="x")
            steps["editMessageText:not-own-A"] = {"status": st, "ok": e3.get("ok"), "description": e3.get("description")}
            obs["edit_foreign_message_rejected"] = not e3.get("ok")
        # (c) chat B: General topic and a second topic
        self.step = "c-chat-B"
        st, g1, _ = await self.call("sendMessage", chat_id=self.chat_b, text="spike general-topic message")
        gr = g1.get("result", {})
        steps["sendMessage:general-B(omitted-thread)"] = {"status": st, "message_id": gr.get("message_id"), "message_thread_id": gr.get("message_thread_id"), "is_topic_message": gr.get("is_topic_message")}
        ...
        st, g2, _ = await self.call("sendMessage", chat_id=self.chat_b, message_thread_id=1, text="spike general-topic message (thread id 1)")
        gr2 = g2.get("result", {})
        steps["sendMessage:general-B(thread-1)"] = {"status": st, "ok": g2.get("ok"), "message_id": gr2.get("message_id"), "message_thread_id": gr2.get("message_thread_id"), "description": g2.get("description")}
        obs["general_topic_accepts_thread_id_1"] = bool(g2.get("ok"))
        if gr2.get("message_id"):
            self.created.append((self.chat_b, gr2["message_id"]))
        st, t2, _ = await self.call("createForumTopic", chat_id=self.chat_b, name=f"agent-colab-spike-{stamp}-b")
        topic_b = t2.get("result", {}).get("message_thread_id")
        steps["createForumTopic:chat-B"] = {"status": st, "ok": t2.get("ok"), "message_thread_id": topic_b, "error": t2.get("description")}
        if topic_b:
            self.topics.append((self.chat_b, topic_b))
            st, b1, _ = await self.call("sendMessage", chat_id=self.chat_b, message_thread_id=topic_b, text="spike topic-B message")
            br = b1.get("result", {})
            steps["sendMessage:topic-B"] = {"status": st, "message_id": br.get("message_id"), "message_thread_id": br.get("message_thread_id")}
            ...
            if gr.get("message_id"):
                st, x1, _ = await self.call("sendMessage", chat_id=self.chat_b, message_thread_id=topic_b, text="cross-topic reply probe", reply_parameters={"message_id": gr["message_id"]})
                xr = x1.get("result", {})
                steps["sendMessage:cross-topic-reply-B"] = {"status": st, "ok": x1.get("ok"), "message_thread_id": xr.get("message_thread_id"), "reply_to_present": "reply_to_message" in xr, "description": x1.get("description")}
                obs["cross_topic_reply"] = {"ok": bool(x1.get("ok")), "lands_in_thread": xr.get("message_thread_id")}
                if xr.get("message_id"):
                    self.created.append((self.chat_b, xr["message_id"]))
        # (d) update shape
        self.step = "d-updates"
        st, u, _ = await self.call("getUpdates", timeout=0, allowed_updates=["message", "edited_message"])
        ...
        obs["bot_own_messages_in_getUpdates"] = any(
            str((up.get("message") or {}).get("from", {}).get("id")) == self.red.bot_id for up in ups
        )
        ...
        async def send(i: int) -> None:
            async with sem:
                ...
                ra = b.get("parameters", {}).get("retry_after") if isinstance(b.get("parameters"), dict) else None
                results.append({"i": i, "status": st, "retry_after": ra, "t": round(time.time() - t_start, 3), "elapsed_s": el})
        ...
        steps["burst40"] = {"total_s": burst_s, "sent_ok": len(ok), "http_429": len(n429), "retry_after_values": sorted({r["retry_after"] for r in n429 if r["retry_after"] is not None}), "results": results}
        obs["burst_40_messages"] = {"ok": len(ok), "http_429": len(n429), "total_s": burst_s, "throughput_msg_per_s": round(len(ok) / burst_s, 2) if burst_s else None,
                                     "max_retry_after_s": max((r["retry_after"] or 0) for r in n429) if n429 else 0}
        if n429:
            wait = max((r["retry_after"] or 1) for r in n429)
            await asyncio.sleep(min(wait, 60))
            st, after, _ = await self.call("sendMessage", chat_id=target_chat, text="after retry_after", **({"message_thread_id": target_thread} if target_thread else {}))
            steps["send-after-retry_after"] = {"status": st, "ok": after.get("ok"), "waited_s": min(wait, 60)}
            if after.get("ok"):
        ...
        for chat, tid in self.topics:
            ...
            st2, d2, _ = await self.call("deleteForumTopic", chat_id=chat, message_thread_id=tid)
            topic_results.append({"chat": self.label(chat), "thread_id": tid, "close_ok": c.get("ok"), "delete_ok": d2.get("ok"), "delete_error": d2.get("description")})
        steps["topics-cleanup"] = topic_results
        obs["delete_topic_possible"] = all(r["delete_ok"] for r in topic_results) if topic_results else None
        self.summary["finished_at"] = _now()
        (EVIDENCE / "summary.json").write_text(json.dumps(self.red.value(self.summary), indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
        self.calls.close()
